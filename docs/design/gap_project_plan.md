...
- Build a consumer that reads events and measures end-to-end latency
  ...
  - **`--isolation-level` knob wired — DONE (currently inert).** Consumer exposes `--isolation-level {read_uncommitted,read_committed}`, symmetric to the producer's `--eos`. With today's non-transactional default producer there is nothing for `read_committed` to filter, so it is wired as a config knob now and becomes functional once a transactional producer ships — flipping the *default* to `read_committed` is deferred to the EOS bullet below.
